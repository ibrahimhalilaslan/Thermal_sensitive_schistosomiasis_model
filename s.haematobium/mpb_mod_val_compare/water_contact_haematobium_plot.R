...
pdf(file = "water_contact_haematobium_mpb.pdf", width = 5, height = 5)
ggplot() +
...
pdf(file = "water_contact_haematobium_prev.pdf", width = 5, height = 5)
ggplot() +
  ...
  theme(legend.position="bottom", legend.key = element_rect(fill = "white"), panel.background = element_rect(fill = "white", colour = "black"), text = element_text(size = 17),
        ...
        plot.title = element_text(size=14, face="bold.italic", hjust=0.5)) + 
  # Add mark segment
  annotate("text", x = percent_positive_best_mpb$temperature[which.max(percent_positive_best_mpb$percent_positive)], y = 0.04, label = '^', colour = "blue", size = 5) + 
  annotate("text", x = percent_positive_best_mpb$temperature[which.max(percent_positive_best_mpb$percent_positive)], y = 0.07,  label = round(percent_positive_best_mpb$temperature[which.max(percent_positive_best_mpb$percent_positive)],1), colour = "blue", size = 5)
