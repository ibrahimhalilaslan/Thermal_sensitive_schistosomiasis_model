...
sch_haematobium_ind <- which(ntd_data$parasite_s == "S. haematobium")

sch_haematobium <- ntd_data[sch_haematobium_ind, ]
...
ntd_data_frame <- data.frame(sch_haematobium$percent_pos, sch_haematobium$bio01)



for (i in 1:length(sch_haematobium$bio01)){
  
}

# Save as CSV with specified column names
write.csv(ntd_data_frame,"S_haematobium_ntd.csv", row.names = FALSE)
# Save as CSV with specified column names
write.csv(model_precent_positive_case,"S_haematobium_model.csv", row.names = FALSE)
