...
plot(preds$temp, preds$.fitted, col = 1, las =1,  pch = 21, lwd = 3, ylab = "",  xlab = "", cex.lab = 2, type = "l", cex.axis=2,
     xlim = c(min(c(preds$temp, .x$temp)), max(c(preds$temp, .x$temp))), ylim = c(min(c(preds$.fitted, .x$rate))-1, max(c(preds$.fitted, .x$rate))))


points(temp_prah_77_haem,  rate_prah_77_haem, col = 2, pch = 19)


text(preds$temp[which.min(preds$.fitted)],   1.2,     labels = "^", cex = 1.5, col = 1)
text(preds$temp[which.min(preds$.fitted)],   1.7,     labels = round(preds$temp[which.min(preds$.fitted)],3), cex = 1.5, col = 1)


mtext(text = expression(paste("Temperature (",degree,"C)")), side = 1, line = 4, cex = 2)
mtext(text = expression(mu[m]), side = 2, line = 2, cex = 2.5, las = 1)
