...
prepatent_period <- approxfun(x = temperature$temp, y = fn_sigma_s(temperature)$.fitted)

# Numerically compute the derivative
derivative_prepatent_period <- function(x, h = 1e-6) {
  (prepatent_period(x + h) - prepatent_period(x - h)) / (2 * h)
}


rate_of_change_prepatent_period <- 0.5*derivative_prepatent_period(temperature$temp)
