...
#rate_ches_17 <- c(0.00244925, 0.006901087, 0.01151302, 0.02073716, 0.04642702)

temp_ches_17 <-      35.5
rate_ches_17 <-  0.04642702
...
#temp_has_74_t <- c(   10,        15,     20,       25,      30,       35)
#rate_has_74_t <- log(c(0.00442,   0.00218, 0.00144, 0.00518, 0.03077,  0.09622))

temp_has_74_t <-     35
rate_has_74_t <-  0.09622


temp <- c(temp_ches_17,temp_joubert_86_g, temp_joubert_86_a, temp_has_74_t)
rate <- c(rate_ches_17, rate_joubert_86_g, rate_joubert_86_a, rate_has_74_t)
...
d_fits <- nest(d, data = c(temp, rate)) %>%
  mutate(flinn = map(data, ~nls_multstart(rate~flinn_1991(temp = temp, a, b, c),
                                          data = .x,
                                          iter = c(5,5,5),
...
d_stack <- select(d_fits, -data) %>%
  pivot_longer(., names_to = 'model_name', values_to = 'fit', flinn:weibull)
...
# plot
pdf(file = "mortality_rate_snail_mult_mod_1.pdf")
