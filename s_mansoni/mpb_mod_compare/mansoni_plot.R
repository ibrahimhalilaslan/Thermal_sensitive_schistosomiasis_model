...
ggplot() +
  ...
  geom_ribbon(aes(temperature, ymin = percent_positive_lower_bound, ymax = percent_positive_upper_bound), percent_positive_boot_conf_preds, fill = 'red', alpha = 0.1) +
  geom_point(aes(temp, prev*max(percent_positive_best_mpb$percent_positive)/max(prev)), d, size = 2, alpha = 0.5, colour = "red") + 
  labs(title = "S.mansoni", x = 'Temperature (ºC)',y = "Percent of positive in human")+
  theme(legend.position="bottom", legend.key = element_rect(fill = "white"), panel.background = element_rect(fill = "white", colour = "black"), text = element_text(size = 17),
        axis.text.x = element_text(color="black", size=17), axis.text.y = element_text(color="black", size=17),  panel.border = element_rect(colour = "black", fill=NA, size=1), 
        plot.title = element_text(size=14, face="bold.italic", hjust=0.5)) + 
  # Add mark segment
  annotate("text", x = percent_positive_best_mpb$temperature[which.max(percent_positive_best_mpb$percent_positive)], y = 0.06, label = '^', colour = "red", size = 5) + 
  annotate("text", x = percent_positive_best_mpb$temperature[which.max(percent_positive_best_mpb$percent_positive)], y = 0.09,  label = round(percent_positive_best_mpb$temperature[which.max(percent_positive_best_mpb$percent_positive)],1), colour = "red", size = 5) +
